...
pub fn validate_pro_program(prog: &str, cfg: &AppConfig) -> anyhow::Result<String> {
    ...
    // 2) 文件名锚定匹配：仅接受已知的合法可执行名
    let file_name = std::path::Path::new(prog.trim_matches('"'))
        .file_name()
        .and_then(|s| s.to_str())
        .unwrap_or(prog)
        .to_lowercase();
    ...
    // 3) 裸名 llama-server 必须 PATH 中能找到
